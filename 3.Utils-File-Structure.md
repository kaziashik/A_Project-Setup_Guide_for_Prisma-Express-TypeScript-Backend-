...
How `catchAsync`, `jwt`, and `sendResponse` fit together in a single request lifecycle:
...
5. On failure at any point, `catchAsync` routes the error to `next(error)` → your centralized error handler.

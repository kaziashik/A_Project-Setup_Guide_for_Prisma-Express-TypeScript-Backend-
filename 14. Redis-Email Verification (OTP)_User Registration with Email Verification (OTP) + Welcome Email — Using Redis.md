...
4. [Quick Redis Setup](#4-quick-redis-setup)
5. [The Flow (Diagram)](#5-the-flow-diagram)
...
> Note: the Redis client file is named `redits.ts` in this project (typo
> of "redis" that stuck) — imports look like
> `import { redisClient } from "../../lib/redits";`. Keep this consistent
> across files, or rename it to `redis.ts` cleanly before your next
> project.
...
That's it for setup — same Redis client is reused everywhere via `import { redisClient } from "../lib/redits"`.
